<div class="info_prdct">
    <div class="container">
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            {{--<li class="nav-item " role="presentation">
                <button class="nav-link view-pill abt-tovar active" id="pills-home-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-home" type="button" role="tab" aria-controls="pills-home"
                        aria-selected="true">О товаре
                </button>
            </li>--}}
            <li class="nav-item " role="presentation">
                <button class="nav-link view-pill active" id="pills-profile-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile"
                        aria-selected="false">Отзывы покупателей
                </button>
            </li>
            <li class="nav-item " role="presentation">
                <button class="nav-link view-pill write-comment" id="pills-comment-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-comment" type="button" role="tab" aria-controls="pills-comment"
                        aria-selected="false">Написать отзыв
                </button>
            </li>
        </ul>
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            {{--<div class="tab-pane fade show active abt-tovar" id="pills-home" role="tabpanel"
                 aria-labelledby="pills-home-tab">
                <div class="pr-character">
                    <h3 style="font-weight: bold;">Характеристики</h3>
                    <div>
                        <ul>
                ...
                <div class="pr-rate">
                    <h3 style="font-weight: bold; margin-right: 245px">Отзывы покупателей</h3>
                    <div>
                        <div class="star-rating d-flex">
                            @if ($product->productReviews()->count() > 0)
                                ...
                                <span class="quan-comm bold">{{$product->productReviews()->count()}} отзыв</span>
                            @endif
                        </div>

                        @foreach ($product->productReviews as $review)
                            <div class="comment-to-pr">
                                <div class="abtuser-view d-flex">
                                    <img class="comm-user_ava"
                                         src="{{asset('img/img/icons/user-icon.png', env('APP_SSL'))}}" alt="ico">
                                    <div class="d-flex align-items-center justify-content-center flex-column">
                                        <span>{{$review->user->name}}</span>
                                        <div class="u-rating ml-5">
                                            @include('components.stars', ['rate' => $review->rate])
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="comment-btns">
                            <a href="javascript:void(0)" class="btn ord-time bold go-to-comment"
                               style="padding: 6px 12px; color: white !important;">Написать отзыв</a>
                        </div>
                    </div>
                </div>
            </div>--}}
            ...
            <div class="tab-pane fade show active" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                <div class="d-flex pre-footer__view">
                    <div class="product-reviews__left col-4">
                        <h3 class="bold">Отзывы о товаре</h3>
                        <div class="star-rating d-flex mt-5 mb-5">
                            @if ($product->productReviews()->count() > 0)
                                <div class="u-rating">
                                    @include('components.stars', ['rate' => $product->productReviews()->sum('rate')/$product->productReviews()->count()])
                                </div>
                    ...
                    <div class="product-reviews__right col-8">
                        <div class="comment-to-pr">
                            @foreach ($product->productReviews as $review)
                                <div class="abtuser-view d-flex align-items-center">
                                    <img class="comm-user_ava"
                                         src="{{asset('img/img/icons/user-icon.png', env('APP_SSL'))}}" alt="ico">
            ...
            <div class="tab-pane fade write-comment" id="pills-comment" role="tabpanel"
                 aria-labelledby="pills-comment-tab">
                @if (!Auth::guard('web')->guest())
                    <form action="{{route('rate.save', ['product' => $product->id])}}"
                          method="post" class="">
                        @csrf
                        <div class="row">
                            <div class="col-6 py-4">
                                <label for="multipleInput" class="form-label bold" style="font-size: 22px;">Если вам необходимо отправить фото!</label>
                                <input type="file" class="form-control h-auto" multiple id="multipleInput" name="file">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label for="validationTextarea" class="form-label bold" style="font-size: 22px;">
                                Отзыв
                            </label>
                            <textarea class="form-control" id="validationTextarea"
                                      style="height: 200px" placeholder="Обязательный пример текстового поля"
                        ...
                        <div class="d-flex justify-content-center">
                            <div class="feedback">
                                <div class="rating">
                                    <input type="radio" name="rating" value="1" id="rating-5" style="display: none">
                                    <label for="rating-5"></label>
                                    <input type="radio" name="rating" value="2" id="rating-4" style="display: none">
                                    <label for="rating-4"></label>
                                    <input type="radio" name="rating" value="3" id="rating-3" style="display: none">
                                    <label for="rating-3"></label>
                                    <input type="radio" name="rating" value="4" id="rating-2" style="display: none">
                                    <label for="rating-2"></label>
                                    <input type="radio" name="rating" value="5" id="rating-1" style="display: none">
                        ...
                        <div class="mb-3 d-flex justify-content-end">
                            <button class="btn ord-time bold go-to-comment" type="submit"
                                    style="color: white !important; ">Отправить
                            </button>
                        </div>
                    </form>
                @else
                    <div class="container-lg">
                        <h4 class="fw-bold text-danger py-5 text-center">Чтобы оставить комментарий вы должны зарегистрироваться или войти в аккаунт</h4>
                    </div>
                @endif
